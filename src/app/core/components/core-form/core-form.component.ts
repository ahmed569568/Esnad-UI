...
export class CoreFormComponent implements OnInit, OnDestroy {
		...
		for (const field of this.service.featureProps) {
			if (field.formField) {
				formFields = {
					...formFields,
					[field.name]: [
						field.initValue ? field.initValue : '',
						field.validations ? field.validations : null
					]
				};
			}
		}
		...
		for (const fieldName of this.imageFieldName) {
			if (fieldName === this.currentField) {
				this.form.controls[fieldName].setValue(
					this.imagePath(fieldName, base64)
				);
			}
		}
	}
	...
	formSubmission(skipLocationChange: boolean = false) {
		...
		if (this.isEdit) {
			return this.service
				.updateItem(skipLocationChange, this.itemId, this.form.value)
				.then(() => {
